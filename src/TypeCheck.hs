...
checkExpr table (BinaryOperation (ArithmeticOperation something)) =
  let (t1, t2) = case something of
   ...
   in if t1' == Just TypeInt && t2' == Just TypeInt
        then Just TypeInt
        else Nothing
...
checkSimpleStm (Just table) (AssignOperation (Assign id expr)) =
  case Map.lookup id table of
    Nothing -> Nothing
    Just t ->
      let t1 = checkExpr (Just table) expr
       in if (Just t) == t1
            then Just table
            else Nothing
...
declareFunc :: Maybe SymTable -> Definition -> Maybe SymTable
declareFunc Nothing _ = Nothing
declareFunc (Just table) (FuncDef t id args stm) =
  case Map.lookup id table of
    Nothing -> Just (Map.insert id t table)
    _ -> Nothing

-- Checks Functions definitions
checkFunc :: Maybe SymTable -> Definition -> Maybe SymTable
...
checkFuncArgs :: Maybe SymTable -> [Parameter] -> Maybe SymTable
...
auxCheckProgram :: Maybe SymTable -> [Definition] -> Maybe SymTable
auxCheckProgram Nothing _ = Nothing
auxCheckProgram table [] = table
auxCheckProgram table (x : xs) =
  let table' = declareFunc table x
   in case checkFunc table' x of
        Nothing -> Nothing
