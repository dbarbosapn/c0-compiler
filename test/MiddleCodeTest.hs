...
prop_relational_expr = transProgram (parse (getTokens "int main(int x, int y) { bool b = x < y; if(b) {return 0;} return 1; }")) === [Def "main" ["t0","t1"] [MOVE "t3" "t0",MOVE "t4" "t1",COND "t3" R_LESS "t4" "L0" "L1",LABEL "L0",MOVEI "t2" 1,LABEL "L1",MOVEI "t2" 0,MOVE "t5" "t2",COND "t5" R_GREATER "zero" "L2" "L3",LABEL "L2",MOVEI "t6" 0,RETVAL "t6",LABEL "L3",MOVEI "t7" 1,RETVAL "t7"]]
